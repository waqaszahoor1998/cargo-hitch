"""
Matching Filters for Order-Driver Assignment
============================================

This file contains FILTERING FUNCTIONS that check if order-driver matches are feasible.
It enforces business rules and constraints for matching.

EXECUTION ORDER:
===============
This file gets imported by sim/matcher/greedy.py during the import phase.
Filter functions are called during the matching process.

KEY FUNCTIONS IN THIS FILE:
==========================
- filter_feasible_matches(): Filters out infeasible order-driver pairs
- is_feasible_match(): Checks if a single order-driver pair is feasible
- _check_distance_constraint(): Validates distance constraints
- _check_capacity_constraint(): Validates capacity constraints
- _check_time_constraint(): Validates time constraints

FILES THAT USE THESE FUNCTIONS:
==============================
- sim/matcher/greedy.py: Calls filter functions before matching
- sim/engine.py: May call filter functions directly

CHRONOLOGICAL USAGE:
===================
1. sim/matcher/greedy.py imports filter functions
2. greedy_matching() calls filter_feasible_matches()
3. filter_feasible_matches() calls is_feasible_match() for each pair
4. is_feasible_match() checks distance, capacity, and time constraints
5. Only feasible matches are considered for assignment

FILTERING CONSTRAINTS:
=====================
- Distance: Maximum detour distance allowed
- Capacity: Driver vehicle capacity limits
- Time: Order delivery time windows
- Bundle Size: Maximum orders per driver
- Service Level: Order service level requirements
"""

from typing import List, Set, Tuple, Dict
from datetime import datetime
from ..entities import Order, Driver  # Data models for filtering
from ..config import MAX_DETOUR_KM, calculate_distance  # Configuration constraints
